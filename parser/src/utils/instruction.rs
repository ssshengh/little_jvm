...
impl Instruction {
    ...
    pub fn parse(raw_code: &[u8], address: usize) -> ClassFileParserResult<(Self, usize)> {
        ...
        let op_code = match op_byte {
            ...
            0xba => {
                ...
                if Self::read_u16(raw_code, &mut address)? != 0 {
                    return Err(ClassFileParserError::InvalidInstructionData("invoke dynamic".to_string(), address));
                }
            ...
            0xb9 => {
                ...
                if Self::read_u8(raw_code, &mut address)? != 0 {
                    return Err(ClassFileParserError::InvalidInstructionData(
                        "invoke interface".to_string(),address
                    ));
                }
            ...
            0xbc => {
                ...
                let array_type = match array_type_byte {
                    ...
                    _ => {
                        return Err(ClassFileParserError::InvalidInstructionData(format!(
                            "newarray: {array_type_byte:#04x}"), address
                        ))
                    }
                };
            ...
            _ => {
                return Err(
                    ClassFileParserError::InvalidInstructionData(format!(
                        "code: {op_byte:#04x}"), address)
                )
            }
        };
    ...
    pub(crate) fn parse_instructions(
        raw_code: &[u8],
    ) -> ClassFileParserResult<Vec<(usize, Instruction)>> {
    ...
    fn byte_at(raw_code: &[u8], address: usize) -> ClassFileParserResult<u8> {
        let op_byte = *raw_code
            .get(address)
            .ok_or(
                ClassFileParserError::CanNotReadInstructionError(address)
            )?;
        Ok(op_byte)
    }

    fn read_u8(raw_code: &[u8], address: &mut usize) -> ClassFileParserResult<u8> {
        let byte = Self::byte_at(raw_code, *address).map_err(|_| {
            ClassFileParserError::CanNotFindArgumentsError(*address)
        })?;
        *address += 1;
        Ok(byte)
    }

    fn read_i8(raw_code: &[u8], address: &mut usize) -> ClassFileParserResult<i8> {
        let value = Self::read_u8(raw_code, address)?;
        Ok(unsafe { std::mem::transmute(value) })
    }

    fn read_u16(raw_code: &[u8], address: &mut usize) -> ClassFileParserResult<u16> {
        let b1 = Self::read_u8(raw_code, address)? as u16;
        let b2 = Self::read_u8(raw_code, address)? as u16;
        Ok((b1 << 8) | b2)
    }

    fn read_i16(raw_code: &[u8], address: &mut usize) -> ClassFileParserResult<i16> {
        let value = Self::read_u16(raw_code, address)?;
        Ok(unsafe { std::mem::transmute(value) })
    }

    fn read_offset(raw_code: &[u8], address: &mut usize) -> ClassFileParserResult<u16> {
        ...
        u16::try_from(jump_address).map_err(|_| {
            ClassFileParserError::InvalidOffsetError(*address)
        })
    }
}
